...
function App() {
  ...
  const [isLoggedIn, setIsLoggedIn] = useState(() => {
    try {
      const stored = localStorage.getItem('zifeng_novel_user');
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed && parsed.username) {
          if (parsed.loginExpiresAt) {
            return new Date() <= new Date(parsed.loginExpiresAt);
          }
          return true;
        }
      }
  ...
  useEffect(() => {
    const loadUserInfo = async () => {
      ...
      if (userInfo) {
        if (userInfo.loginExpiresAt) {
          const now = new Date();
          const expiresAt = new Date(userInfo.loginExpiresAt);
          if (now > expiresAt) {
            clearUserInfo();
            setIsLoggedIn(false);
            setUserInfo(null);
  ...
  useEffect(() => {
    const interval = setInterval(async () => {
      ...
      if (latestUserInfo) {
        // 检查登录是否过期
        const now = new Date();
        const expiresAt = new Date(latestUserInfo.loginExpiresAt);
        if (now > expiresAt) {
          // 登录已过期，清除登录状态
          clearUserInfo();
          setIsLoggedIn(false);
          setUserInfo(null);
        } else if (JSON.stringify(latestUserInfo) !== JSON.stringify(userInfo)) {
          // 用户信息有更新，更新状态
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 20, flex: '1 1 auto', minWidth: 0 }}>
          ...
          <Menu
            ...
            items={[
              { key: '/', icon: <HomeOutlined />, label: <NavLink to="/" style={{ textDecoration: 'none' }}>首页</NavLink> },
              { key: '/category', icon: <AppstoreOutlined />, label: <NavLink to="/category" style={{ textDecoration: 'none' }}>分类</NavLink> },
              { key: '/shelf', icon: <BookOutlined />, label: <NavLink to="/shelf" style={{ textDecoration: 'none' }}>书架</NavLink> },
              { key: '/setting', icon: <SettingOutlined />, label: <NavLink to="/setting" style={{ textDecoration: 'none' }}>设置</NavLink> },
            ]}
          />
        </div>
