...
import { Button, Spin, Select, Divider, Slider, Modal, message, Tooltip, Drawer } from 'antd';
import { 
...
const Reader = () => {
  ...
  const tocContainerHeight = typeof window !== 'undefined' ? window.innerHeight - 55 : 400;
  const handleTocScroll = (e) => { setTocScrollTop(e.target.scrollTop); };
  const visibleStartIndex = Math.max(0, Math.floor(tocScrollTop / ITEM_HEIGHT) - BUFFER);
  const visibleEndIndex = Math.min(chapters.length, Math.ceil((tocScrollTop + tocContainerHeight) / ITEM_HEIGHT) + BUFFER);
  ...
  useEffect(() => {
    if (showToc && tocContainerRef.current) {
      const scrollTo = currentChapterIndex * ITEM_HEIGHT - tocContainerHeight / 2 + ITEM_HEIGHT / 2;
      tocContainerRef.current.scrollTop = Math.max(0, scrollTo);
      setTocScrollTop(tocContainerRef.current.scrollTop);
    }
  }, [showToc]);
  ...
  }, [isLoggedIn, userInfo, currentChapter, chapters.length, bookUrl, novelId, currentChapterIndex, bookData, sourceUrl]);
  ...
  if (loading && !content) {
    return (
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', backgroundColor: isDarkMode ? '#1a1a2e' : '#ffffff' }}>
        <Spin size="large" />
      </div>
    );
  }
  ...
  return (
    <div ref={pageRef} style={{ position: 'relative', minHeight: '100vh', '--reader-text-color': readerSettings.textColor, ...bgStyle }}>
          ...
          <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.fontSize}px</span>
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.fontWeight}</span>
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.indent}字符</span>
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.lineHeight}</span>
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.paragraphSpacing}px</span>
              </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.letterSpacing}px</span>
              </div>
          ...
          <div style={{
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                ...
                <span style={{ marginLeft: 'auto', color: isDarkMode ? '#888' : '#999', fontSize: windowWidth < 768 ? 12 : 13 }}>{readerSettings.textColor}</span>
              </div>
              <input type="color" value={readerSettings.textColor} onChange={handleTextColorChange} onBlur={() => handleColorPickerClose('text')} style={{ width: windowWidth < 768 ? '100%' : 60, height: 32, border: `1px solid ${isDarkMode ? '#444' : '#d9d9d9'}`, borderRadius: 6, cursor: 'pointer' }} />
            </div>
            ...
            <div>
                ...
                {bgPresets.map((preset) => (
                  <div
                    ...
                    style={{
                      ...
                      border: readerSettings.bgColor === preset.color && !readerSettings.bgImage ? `2px solid ${color}` : `1px solid ${isDarkMode ? '#444' : '#d9d9d9'}`,
                      display: 'flex',
                      alignItems: 'center',
                      justifyContent: 'center',
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                <span style={{ fontSize: windowWidth < 768 ? 12 : 13, color: isDarkMode ? '#aaa' : '#666' }}>自定义颜色：</span>
                <input type="color" value={readerSettings.bgColor} onChange={handleBgColorChange} onBlur={() => handleColorPickerClose('bg')} style={{ width: 40, height: 30, border: `1px solid ${isDarkMode ? '#444' : '#d9d9d9'}`, borderRadius: 4, cursor: 'pointer' }} />
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginTop: 12, flexWrap: 'wrap' }}>
                <span style={{ fontSize: windowWidth < 768 ? 12 : 13, color: isDarkMode ? '#aaa' : '#666' }}>背景图片</span>
                <label style={{ padding: '4px 12px', borderRadius: 6, border: `1px solid ${isDarkMode ? '#444' : '#d9d9d9'}`, cursor: 'pointer', fontSize: windowWidth < 768 ? 12 : 13, transition: 'all 0.2s' }}>
                  选择图片
            ...
            <Button
              onClick={() => {
                setReaderSettings({
                  fontSize: 16, fontWeight: 400, lineHeight: 2, paragraphSpacing: 16,
                  indent: 2, letterSpacing: 0, bgColor: isDarkMode ? '#1a1a2e' : '#ffffff', textColor: isDarkMode ? '#e0e0e0' : '#333333', bgImage: ''
                });
      ...
      <Drawer
        ...
        width={windowWidth <= 480 ? '100%' : windowWidth <= 768 ? 280 : 300}
        styles={{ body: { padding: 0 } }}
      >
        <div
          onScroll={handleTocScroll}
          style={{ height: tocContainerHeight, overflowY: 'auto' }}
          ref={tocContainerRef}
        >
